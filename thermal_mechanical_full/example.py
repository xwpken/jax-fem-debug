...
class thermal_mechanical_full(Problem):
    ...
    def set_params(self, params):
        sol_du_old, sol_dT_old, dt = params
        self.internal_vars = [self.fe_u.sol_to_grad(sol_du_old), 
    ...
    def get_universal_kernel(self):
        ...
        def universal_kernel(cell_sol_flat, x, cell_shape_grads, cell_JxW, cell_v_grads_JxW, u_grads_old, T_old, dt):
            ...
            ## Handles the term 'k * inner(grad(T^n),grad(p)) * dx'
...
def main_fns():
    ...
    dirichlet_bc_info_T = [[circle],[0],[theta_dirichlet]]
    ...
    # vtk_path_u = os.path.join(output_dir, f'vtk/jax-fem_u.vtk')
    # vtk_path_dT = os.path.join(output_dir, f'vtk/jax-fem_theta.vtu')
    # save_sol(problem.fes[0], sol_list[0], vtk_path_u)
    # save_sol(problem.fes[1], sol_list[1], vtk_path_dT)
