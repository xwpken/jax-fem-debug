...
"""
The bracket operator. One may define something like 'lambda x: np.maximum(x, 0.)' 
...
"""
Define the phase field variable class. Note how 'get_tensor_map' and 'get_mass_map'
specify the corresponding terms in the weak form. Since the displacement variable u 
affects the phase field variable d through the history variable H, we need to set 
this using set_params.
"""
...
"""
Define the displacement variable class. As we previously discussed, 
native JAX AD may return NaN in the cases with repeated eigenvalues. 
We provide two workarounds and users can choose either one to use. 
The first option adds a small noise to the strain tensor, 
while the second option defines custom derivative rules to properly 
